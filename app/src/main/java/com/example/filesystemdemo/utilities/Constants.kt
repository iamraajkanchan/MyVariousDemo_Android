package com.example.filesystemdemo.utilities

const val TAG: String = "FileSystemDemo"
const val ARG_FILE_NAME: String = "file name argument"
const val ARG_FOLDER_NAME: String = "folder name argument"
const val EMPTY_FILE_ERROR: String = "File is empty!!!"
const val FILE_NOT_EXIST_ERROR: String = "File doesn't exist!!!"
const val FOLDER_NOT_EXIST_ERROR: String = "Folder doesn't exist!!!"
const val NOTIFICATION_NAME = "FileSystem"
const val NOTIFICATION_CHANNEL = "FileSystem_channel_01"
const val NOTIFICATION_WORK = "FileSystem_notification_work"
const val NOTIFICATION_TITLE = "FileSystem Log Service"